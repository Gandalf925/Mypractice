...
- Public GitHub directory: not modified
...
A headless Chromium smoke test was attempted against both local HTTP and local file URLs. Chromium was started successfully, but the execution environment replaced the page with an organization-policy block page stating that local content was not allowed. The block occurred before application HTML loaded, so it is not evidence of an application error.
...
4. For GitHub publication, use the agreed normal Git/Git Credential Manager workflow with one commit and one push.
