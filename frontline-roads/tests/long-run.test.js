...
test('twenty-four-hour simulation remains bounded, serializable and within enemy cap', () => {
  const state = largeConnectedState();
  const simulator = new OfflineSimulator({
    combatSystem: new CombatSystem(),
    civilizationSystem: new CivilizationSystem()
  });
  const summary = simulator.simulate(state, 24 * 60 * 60);
  assert.ok(Math.abs(summary.simulatedSeconds - 24 * 60 * 60) < 0.001);
  assert.ok(summary.iterations <= 400000);
