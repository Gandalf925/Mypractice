...
test('all defense tiers define deterministic durability and sequential upgrade costs', () => {
  for (const line of ['barrier', 'single', 'area', 'slow', 'repair']) {
    assert.equal(DEFENSE_LINES[line].length, 8);
...
test('destroyed and maximum-tier defenses cannot be upgraded', () => {
  const state = makeState();
  state.civilization.level = 7;
  Object.assign(state.inventory.resources, { timber: 100, rope: 100, stone: 100, cutStone: 100, wroughtIron: 100, steel: 100, mechanism: 100 });
  const destroyed = gunDefense({ hp: 0 });
  assert.match(defenseUpgradeStatus(state, destroyed).reason, /撤去済み/);
  const maximum = gunDefense({ tier: 7, defenseKey: 'single7', hp: 680, maxHp: 680 });
...
test('gate conversion starts at tier two and preserves the prior health ratio', () => {
  ...
  assert.equal(barrier.maxHp, 500);
  assert.equal(barrier.hp, 250);
});
