...
test('periodic HUD refresh shares one detached state snapshot across all UI panels', async () => {
  ...
  assert.match(bootstrap, /onUiUpdate: \(\) => \{\s*const view = this\.store\.uiSnapshot\(\);\s*this\.combatUi\.update\(view\);\s*this\.deploymentUi\.update\(view\);\s*this\.baseCommandUi\.update\(view\);\s*this\.civilizationUi\.update\(view\);/s);
});
