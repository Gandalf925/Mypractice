...
test('existing facilities do not expand construction beyond bases and the player', () => {
  ...
  assert.deepEqual(build.listBuildSites(state, 'barrier').map(site => site.edgeId).sort(), ['cross', 'near-far']);
});
...
test('barrier candidates use one stable road-section position and reject out-of-range points', () => {
  const state = makeState();
  const build = new BuildSystem();

  const valid = build.previewAt(state, 'barrier', { x: 0, y: 42 }, 5);
  const invalid = build.previewAt(state, 'barrier', { x: 120, y: 0 }, 5);

  assert.equal(valid.ok, true);
  assert.equal(valid.candidate.edgeId, 'cross');
  assert.deepEqual(valid.candidate.point, { x: 0, y: 40 });
...
test('preview skips an occupied nearest road when another valid road is within the tap tolerance', () => {
  ...
  const result = new BuildSystem().previewAt(state, 'barrier', { x: 30, y: 25 }, 30);

  assert.equal(result.ok, true);
  assert.equal(result.candidate.edgeId, 'available-road');
  assert.deepEqual(result.candidate.point, { x: 30, y: 50 });
});
