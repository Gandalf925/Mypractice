...
import { fieldBaseMaxHpForCivilization } from '../src/base/field-bases.js';
...
function fixture() {
  ...
  state.combat.defenses = [{ id: 'tower', kind: 'tower', type: 'gun', nodeId: 'remote', hp: 100, maxHp: 100 }];
...
test('base command UI lists simple bases separately and can focus or rebuild a destroyed one', async () => {
  ...
  try {
    ...
    assert.equal(state.world.fieldBases[0].hp, fieldBaseMaxHpForCivilization(state.civilization.level));
  } finally { globalThis.document = prior; }
});
