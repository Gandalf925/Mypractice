...
test('release version is synchronized across package, runtime label and service-worker cache', async () => {
  ...
  const releaseLoader = await readFile(resolve(root, 'src/app/release-loader.js'), 'utf8');
  ...
  assert.match(releaseLoader, new RegExp(`RELEASE_ID = ['"]${runtimeVersion}['"]`));
  assert.ok(releaseLoader.includes(`bootstrap.js?v=${packageData.version}`));
});
