...
test('base construction ranges grow gradually with a hard cap while the current-position radius stays fixed', () => {
  const expected = [
    ...
    { level: 5, major: 285, field: 205 },
    { level: 6, major: 315, field: 230 },
    { level: 7, major: 345, field: 255 }
  ];
  ...
  assert.equal(majorBaseBuildRange(99), 345);
  assert.equal(fieldBaseBuildRange(99), 255);
});
...
test('civilization growth does not turn the player position into an unlimited mobile construction anchor', () => {
  ...
  assert.equal(sites.some(site => site.nodeId === 'player' && site.anchorKind === 'PLAYER'), true);
