...
export function operationGuidanceMarkup(guidance, i18n = null) {
  const operations = guidance?.operations ?? [];
  const walkTargets = guidance?.walkTargets ?? [];
  const opHtml = operations.length
    ? operations.map(item => `<article class="opsCard"><div><span>${esc(item.tag)}</span><strong>${esc(item.title)}</strong><small>${esc(item.detail)}</small></div>${actionKey(item.action) ? `<button type="button" class="opsActionButton" data-operation-action="${esc(actionKey(item.action))}" data-operation-id="${esc(item.id)}">${esc(actionLabel(item.action))}</button>` : ''}</article>`).join('')
    : `<p class="emptyText">${esc(text(i18n, 'operation.noUrgentOps', {}, '現在は緊急の作戦目標はありません。周辺の敵・物資・文明条件を確認してください。'))}</p>`;
  const walkHtml = walkTargets.length
    ? walkTargets.map(item => `<article class="walkTargetCard"><span>${esc(item.kind)}</span><strong>${esc(item.title)}</strong><small>${esc(item.detail)}</small></article>`).join('')
    : `<p class="emptyText">${esc(text(i18n, 'operation.noWalkTargets', {}, '現在地周辺に優先表示する徒歩目標はありません。'))}</p>`;
  return `<section class="opsSummary"><h2>${esc(text(i18n, 'operation.nextOpsHeading', {}, 'NEXT OPS // 次の行動'))}</h2><p class="sectionNote">${esc(text(i18n, 'operation.nextOpsNote', {}, '現在の状況から、次に有効な行動を優先順に表示します。'))}</p><div class="opsGrid">${opHtml}</div></section><section class="opsSummary"><h2>${esc(text(i18n, 'operation.walkTargetsHeading', {}, 'WALK TARGETS // 近くの目標'))}</h2><div class="walkTargetGrid">${walkHtml}</div></section>`;
}
