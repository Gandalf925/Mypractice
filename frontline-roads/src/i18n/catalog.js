...
function applyVariantPatterns(language, value, variant) {
  ...
  if (variant === 'status') {
    ...
    if (match) return normalized === 'ko' ? `거점 저장됨 · ${match[1]}m` : normalized === 'zh' ? `基地已保存 · ${match[1]}m` : normalized === 'en' ? `Base saved · ${match[1]}m` : text;
    match = text.match(/^拠点設置完了：初回現在地から約(\d+)m/);
    if (match) return normalized === 'ko' ? `거점 설치됨 · ${match[1]}m` : normalized === 'zh' ? `基地设置完成 · ${match[1]}m` : normalized === 'en' ? `Base placed · ${match[1]}m` : text;
...
function scrubResidualSourceLanguage(language, value) {
  ...
  if (normalized === 'ko') return scrubResidualJapaneseForKorean(value);
  return String(value ?? '');
}
...
export function bundleTextLocalized(language, bundle = {}, { compact = false } = {}) {
  ...
  if (compact && (normalized === 'en' || normalized === 'zh' || normalized === 'ko')) {
...
export function isSupportedLanguage(language) {
  ...
  return SUPPORTED_LANGUAGE_CODES.includes(code) && Boolean(TEXT_CATALOG[code]);
}
