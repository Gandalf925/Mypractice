...
function pruneStaleFrontlineEnemyNetwork(state) {
  ...
  for (const base of state.world?.enemyBases ?? []) {
    ...
    base.retired = true;
    base.retiredAt = state.runtime?.worldTimeMs ?? Date.now();
  }
...
function uniqueEnemyBaseId(state, type, idSeed) {
  ...
  return stableId('enemy_base', type, idSeed, state.runtime?.worldTimeMs ?? Date.now(), Math.random());
}

function createBase(state, type, placement, idSeed = placement.node.id, metadata = {}) {
