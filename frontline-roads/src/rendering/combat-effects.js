...
function ring(context, point, radius, color, alpha, width = 1.5, dashed = false) {
  context.save();
...
function cross(context, point, radius, color, alpha) {
  context.save();
  context.globalAlpha = alpha;
  context.strokeStyle = color;
  context.shadowColor = color;
  ...
  draw(context, camera, state, timeMs, width, height, preferences = {}) {
    ...
    for (const effect of effects) {
      ...
      if (effect.type === 'shot' && payload.from && payload.to) {
        ...
        context.stroke();
        context.restore();
        ring(context, { x, y }, 2.5 + progress * 4, color, fade, 1);
        continue;
      }
      ...
      if (effect.type === 'explosion') {
        ...
        ring(context, point, radius, '#ffb35f', fade * 0.85, 2);
        ring(context, point, radius * 0.58, '#ffe0a6', fade * 0.7, 1);
      } else if (effect.type === 'kill') {
        ring(context, point, 5 + progress * 24, '#ff7588', fade * 0.8, 1.4, true);
        cross(context, point, 3 + progress * 8, '#ff9aaa', fade * 0.75);
      } else if (effect.type === 'cityHit') {
        screenAlert = Math.max(screenAlert, fade * 0.72);
        ring(context, point, 18 + progress * 42, '#ff5268', fade, 2, true);
      } else if (effect.type === 'cityDefeated') {
        screenAlert = Math.max(screenAlert, fade);
        ring(context, point, 20 + progress * 90, '#ff284d', fade, 3, true);
        ring(context, point, 12 + progress * 56, '#ff9aaa', fade * 0.8, 1.5);
      } else if (effect.type === 'defenseBuilt') {
        ring(context, point, 7 + progress * 28, '#65ffd0', fade * 0.9, 1.4);
      } else if (effect.type === 'defenseRepaired') {
        ring(context, point, 5 + progress * 22, '#8affdf', fade * 0.9, 1.2, true);
      } else if (effect.type === 'defenseUpgraded') {
        ring(context, point, 7 + progress * 30, '#65d7ff', fade * 0.9, 1.5);
        ring(context, point, 4 + progress * 19, '#bb8cff', fade * 0.72, 1, true);
      } else if (effect.type === 'defenseDestroyed') {
        cross(context, point, 5 + progress * 16, '#ff5268', fade);
        ring(context, point, 8 + progress * 28, '#ff5268', fade * 0.75, 1.4, true);
      } else if (effect.type === 'waveLaunched') {
        ring(context, point, 9 + progress * 48, '#ff5268', fade * 0.85, 1.4, true);
        ring(context, point, 5 + progress * 28, '#ff9f43', fade * 0.65, 1);
      }
    }
    context.restore();
