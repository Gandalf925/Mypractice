...
export class SettlementSystem {
  ...
  build(state, type) {
    ...
    if (!isBuildingUnlocked(state, type)) return { ok: false, reason: 'Civilization level is too low.' };
    ...
    if (usedSettlementSlots(state) >= settlementSlotLimit(state) && !sameStorageSlot) return { ok: false, reason: 'No settlement building slot available.' };
    if (definition.limit && existing >= definition.limit) return { ok: false, reason: 'This facility cannot be built.' };
    if (!consumeBundle(state, definition.cost)) return { ok: false, reason: 'Not enough resources.', missing: missingBundle(state, definition.cost) };
    const building = {
  ...
  repair(state, buildingId) {
    ...
    if (missingHp <= 0) return { ok: false, reason: 'Repair Not needed.' };
    ...
    if (!consumeBundle(state, cost)) return { ok: false, reason: 'Not enough resources to repair.', missing: missingBundle(state, cost) };
  ...
  processDamageQueue(state) {
    ...
    for (const incident of queue) {
      ...
      this.events?.emit('message', { text: `${SETTLEMENT_BUILDINGS[destroyed.type]?.name ?? 'Settlement Facilities'} Destroyed, Building slots from Remove.` });
    }
  }
}
