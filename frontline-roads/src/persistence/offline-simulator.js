...
export class OfflineSimulator {
  ...
  simulate(state, elapsedSeconds) {
    ...
    const before = {
      ...
      unavailableDefenses: state.combat.defenses.filter(defense => defense.ruined || defense.hp <= 0).length,
      unavailableBuildings: (state.civilization?.buildings ?? []).filter(building => building.ruined && !building.demolished).length,
    ...
    const unavailableDefenses = state.combat.defenses.filter(defense => defense.ruined || defense.hp <= 0).length;
    const unavailableBuildings = (state.civilization?.buildings ?? []).filter(building => building.ruined && !building.demolished).length;
    ...
    return {
      ...
      defensesLost: unavailableDefenses - before.unavailableDefenses,
      buildingsLost: unavailableBuildings - before.unavailableBuildings,
