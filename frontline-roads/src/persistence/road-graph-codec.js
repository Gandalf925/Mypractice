const ROAD_GRAPH_FORMAT = 'frontline-road-graph-1';
...
export function encodeRoadGraph(graph) {
  ...
  return {
    ...
    nodes: graph.nodes.map(node => [node.id, rounded(node.x), rounded(node.y)]),
...
export function decodeRoadGraph(value) {
  if (!value || value.format !== ROAD_GRAPH_FORMAT) return value;
  return {
