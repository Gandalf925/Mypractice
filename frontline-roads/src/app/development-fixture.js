function gridWays(center) {
  ...
  for (let row = -3; row <= 3; row += 1) {
    ways.push({
      type: 'way', id: id++,
      tags: { highway: row === 0 ? 'primary' : 'residential', name: row === 0 ? 'Central Road' : `East-West Road${row}` },
  ...
  for (let column = -3; column <= 3; column += 1) {
    ways.push({
      type: 'way', id: id++,
      tags: { highway: column === 0 ? 'secondary' : 'residential', name: column === 0 ? 'Central North-South Road' : `North-South Road${column}` },
      geometry: Array.from({ length: 7 }, (_, index) => ({
