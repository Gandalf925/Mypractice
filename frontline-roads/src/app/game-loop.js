export class GameLoop {
  constructor({ store, combatSystem, civilizationSystem = null, renderer, saveRepository, onUiUpdate, onError }) {
    this.store = store;
  ...
  frame(time) {
    ...
    if (this.uiClock >= 0.25) { this.uiClock = 0; this.onUiUpdate?.(); }
    if (this.saveClock >= 5) {
      this.saveClock = 0;
      try { const savedAt = this.saveRepository.save(this.store.getState()); this.store.mutate(state => { state.runtime.lastSavedAt = savedAt; }, 'save:timestamp'); }
